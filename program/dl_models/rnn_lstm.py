...
def embedding_lstm(rq1_data, config, smell, rq1_out_folder=RQ1_OUT_FOLDER, dim = DIM):
...
def get_all_data(data_path, smell):
    ...
    train_data, train_labels, eval_data, eval_labels, max_input_length = \
        inputs.get_data(data_path=data_path)
...
def main(data_path, smell):
    rq1_data = get_all_data(data_path, smell)
    ...
    total_iterations = 5
    #len(emb_outputs) * len(lstms_units) * len(rnn_layers) * len(epochs) * len(dropouts)
    cur_iter = 1
                    ...
                    for dropout in dropouts:
                        ...
                        try:
                            ...
                            rq1_auc, rq1_accuracy, rq1_precision, rq1_recall, rq1_f1, rq1_average_precision, \
                             stopped_epoch \
                                = embedding_lstm(rq1_data, config, smell)
                            end_time = time.time()
...
if __name__ == "__main__":
    ...
    for smell in smell_list:
        ...
        #print(rq2_eval_data_path)

        inputs.preprocess_data_2d(data_path)
        #inputs.preprocess_data_2d(rq2_eval_data_path)
